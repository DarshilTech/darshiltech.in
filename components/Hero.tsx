...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Entrance timeline — selectors scoped within heroRef
  ...
  return (
    ...
    >
      ...
      {/* Meta strip */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-5 md:gap-10 font-mono text-[10px] tracking-[0.1em] text-ink-dim pb-6 border-b border-line uppercase">
        <MetaItem label="// Availability" value="Worldwide" />
        <MetaItem label="// Status" value="● Live" highlight />
        <MetaItem label="// Expertise" value="Magento • Hyvä • Headless" />
        <MetaItem label="// Discipline" value="Commerce Architecture" />
        <MetaItem label="// Foundation" value="2025" />
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-10 lg:gap-15 items-center flex-1 py-10">
        <div>
          <h1
            className="font-display font-light leading-[1] tracking-[-0.03em] mb-8"
          ...
          <p className="hero-sub text-[15px] leading-[1.6] text-ink-dim max-w-[480px] mb-10 font-light">
            DarshilTech is a software studio building scalable enterprise commerce solutions with Magento, Hyvä, Go, and headless architecture. We focus on performance, sustainable engineering, and systems designed for long-term growth.
          </p>
      ...
      {/* Bottom */}
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-10 items-end pt-6 border-t border-line font-mono text-[10px] text-ink-dim uppercase tracking-[0.1em] pb-10">
...
function Word({ children, italic }: { children: React.ReactNode; italic?: boolean }) {
  return (
    <span className="hero-word overflow-hidden">
      <span
